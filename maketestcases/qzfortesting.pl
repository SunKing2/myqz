...
sub DoListStats () {
  ...
  my $age_sum = 0;
  my $now = time;
  print '===int[] absoluteResponseTimes = {';
  print substr($responseAbsoluteTimes, 0, -1);
  print "};===\n";
  print "===int iEndTime = $now;===\n";
  my $count = 0;
...
sub DoRunQuiz(@) { my @argv = @_;
  ...
  $gSessionStart = time;
  print "===int iStartTime = $gSessionStart;===\n";
...
sub GotIt ($$) { my($q, $time) = @_;
  my $oldAge;
  {
    ...
    $gQData[$qIndex] = time;
    #print "=== gotit $oldAge $gQData[$qIndex] ===\n";
  }
...
sub ReadLine ($$) { my ($totaltime, $makelc) = @_;
  ...
  $prompt'waiting = 1;
  $prompt'start = time;
  #print "===ReadLine ps1 $prompt'start ===\n";
  print $prompt'text;
  ...
  while (1) {
    ...
    print "$_";
    print "===$ttt, ===\n";
    $responseAbsoluteTimes = $responseAbsoluteTimes . $ttt . ',';
    $duration = $ttt - $prompt'start - (length $_)/$config'typing_speed;
    last if $_ eq '';
    ...
    $prompt'start = time;
    #print "===ReadLine ps2 $prompt'start ===\n";
    $prompt'waiting = 1;
    print $prompt'text;
    }
  $_[0] += $time;
